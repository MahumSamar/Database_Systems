...
$sql="SELECT * FROM CustomerRequest WHERE 
...
$result = $conn->query($sql);
$row = $result->fetch_assoc();
$comProfitPer=$row['profitPercentage'];


if($result){
    $sql = "INSERT INTO ServiceServed (providerID,categoryID,customerID,month,dateHired,earned,companyProfit) VALUES (?, ?, ?, ?, ?, ?, ?)";
   $stmt = $conn -> prepare( $sql);
   if ($stmt) {
       // the ssssss here are for representing the type of the parameters passed
       $stmt -> bind_param("sssssss", $providerID, $categoryID, $customerID, $month, $dateHired, $earned, $companyProft);
...
$company=$_POST['earned']-($_POST['earned'] * $comProfitPer/100);
echo $company;

$earned=$_POST['earned']-$company;
$companyProft=$company;
...
<head>
    ...
    <title>Service Served Form</title>
    <style>
        table {
                        ...
                        <form action="" method="POST">
                            <div class="signUpForm row">
                                ...
                                <div class="formField col-lg-6">
                                    <input id="earned" class="input-text" type="text" name="earned" required>
                                    <label for="earned" class="label">Total Earned</label>
                                </div>
