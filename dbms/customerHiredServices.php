...
<head>
    ...
    <title>Customer hired Services</title>
    <style>
        table{
    ...
    text-align: center;
        ...
        tr:nth-child(even){background-color: #D4A59A;}
    </style>
</head>
                    ...
                    <div class="container">
                        <form action="" method="POST">
                            <div class="signUpForm row">
                                ...
                                <?php
                                $sql = "SELECT requestID,categoryName, firstName, lastName,phone,email, dateHired, status
                                FROM CustomerRequest CR
                                         INNER JOIN Customer on Customer.customerID = CR.customerID
                                         INNER JOIN Category C on CR.categoryID = C.categoryID
                                         INNER JOIN PersonInfo PI on CR.providerID = PI.personID
                                WHERE CR.customerID = '" . $_SESSION['id'] . "'";
                                ...
                                if ($result) {
                                    ...
                                    while ($row = $result->fetch_assoc()) {
                                        echo "<tr><td>" . $row['requestID'] . "</td><td>" . $row['categoryName'] . "</td><td>" . $row['firstName'] ." ". $row['lastName']
                                            . "</td><td>" . $row['dateHired'] .  "</td><td>" . $row['phone'].  "</td><td>" . $row['email'].  "</td><td>" . $row['status'] . "</td></tr>";
                                    }
                                    echo "<br>";
                                    echo "</table>";
                                    ...
                                    echo "You have not hired any service right now.";
                                }
